# code for multitissue cis eQTL mapping in matrixeQTL
# only the main matrixeqtl fuction was modified following http://www.bios.unc.edu/research/genomic_software/Multi-Tissue-eQTL/code1.html
...
###
  ...
  if (analysis == "G"){
    ...
    G_eQTL = Matrix_eQTL_main(
      ...
      gene = gene,
      cvrt = cvrt,
      output_file_name.cis = paste("multitissue.results.cis.", file, sep=""),
      output_file_name = paste("multitissue.results.trans.", file, sep=""),
    ...
    cat(file="degreesfreedom.multitissue.txt", file, "\t", G_eQTL$param$dfFull, "\n", append=TRUE) 
